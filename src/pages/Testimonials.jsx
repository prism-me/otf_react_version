...
const Testimonials = (props) => {
  ...
  const getAllTestimonial = () => {
    API.get("/testimonials")
      .then((response) => {
        const alltestimonial = response.data?.data?.filter(
          (x) => x.type === "testimonial"
        );
        const allmemberreview = response.data?.data?.filter(
          (x) => x.type === "member_review"
        );
        const videotestimonial = response.data?.data?.filter(
          (x) => x.type === "video_review"
        );
    ...
    <div>
      <Helmet>
        ...
        <meta
          ...
          content={
            global?.activeLanguage === "ar"
              ? testimonialsMetaData?.arabic?.meta_details?.meta_description
              : testimonialsMetaData?.meta_details?.meta_description
          }
        />
      </Helmet>
      <Layout
