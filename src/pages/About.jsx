...
const About = (props) => {
  ...
  const getAllTeams = () => {
    API.get("/teams")
      .then((response) => {
        const allteams = response.data?.data?.filter(
          (x) => x.type === "our_team"
        );
        const allcoaches = response.data?.data?.filter(
          (x) => x.type === "our_coaches"
        );
    ...
    <div>
      <Helmet>
        ...
        <meta
          ...
          content={
            global?.activeLanguage === "ar"
              ? aboutMetaData?.arabic?.meta_details?.meta_description
              : aboutMetaData?.meta_details?.meta_description
          }
        />
      </Helmet>
      <Layout
